...
export function useDeployedContractInfo<TContractName extends ContractName>(
  ...
  const networkChainId = Number(selectedNetwork.id) as number;
  const deployedContract = (contracts as any)?.[networkChainId]?.[contractName as ContractName] as Contract<TContractName>;
